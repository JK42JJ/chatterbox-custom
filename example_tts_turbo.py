...
# Full sleep article text for English learning (6 chunks)
texts = [
...
# Generate each chunk with default Turbo voice and concatenate
all_wavs = []
for i, text in enumerate(texts):
    print(f"Generating chunk {i+1}/{len(texts)}...")
    wav = model.generate(text)  # No audio_prompt_path = default Turbo voice
    all_wavs.append(wav)
    gc.collect()  # Free memory between chunks

# Concatenate all audio
final_wav = torch.cat(all_wavs, dim=1)
ta.save("sleep-turbo-full.wav", final_wav, model.sr)
print(f"Generated: sleep-turbo-full.wav")
